...
public class ClientProxy implements IProxy {
    ...
    @Override
    public void init() {
        if (CCIntegration.isModLoaded())Item.getItemFromBlock(Registries.block_interface_tardis_cc).setTileEntityItemStackRenderer(new RendererInterfaceItem());
        if (OCIntegration.isModLoaded())Item.getItemFromBlock(Registries.block_interface_tardis_oc).setTileEntityItemStackRenderer(new RendererInterfaceItem());
